...
print(salidas_esperadas)
print(salidas_modelos)
# Parejas salida vs esperada
parejas = []
for salida in salidas_modelos:
    for esperada in salidas_esperadas:
        if salida.split("-")[0] == esperada.split("-")[0].replace("respuesta_", ""):
...
for pareja in parejas:
    ...
    salida_modelo = json.loads(salida_modelo)["respuesta"]
...
# Añadir media y desviación típica
df.loc['media'] = df.mean()
df.loc['desviacion'] = df.std()
